...
export default function HomePage() {
  ...
  const handleClick = async (e)=>{
    ...
    try {
      ...
      const res = await axios.post('/api/check-path', { pathInput })

      if(res.data.exist){
        router.push(`/${pathInput}?auth=1`)
      } else {
        router.push(`/${pathInput}?create=1`)
      }
    } catch (error) {
      console.log(error)
    }
  ...
  return (
    ...
    >
      ...
      <p className="text-[#4B5563]">
        Securely store your private text with end-to-end encryption.
      </p>
      ...
      <div className="flex gap-2 items-center mt-18 ">
        ...
        <input
          ...
          placeholder="notes"
        />
        <button onClick={handleClick} className="bg-[#16BC88] px-4 py-1 rounded-md text-white flex items-center gap-2">
